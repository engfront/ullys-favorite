  ...
  if (!product) {
    return (
      <div className="container-x py-32 text-center">
        ...
        <Link href="/ullys/" className="btn-dark mt-8">Kembali ke Beranda</Link>
      </div>
    );
  }
    ...
    <>
      ...
      <main className="container-x py-10 md:py-16">
        ...
        <nav className="text-xs text-black/50 mb-8 space-x-2">
          <Link href="/ullys/" className="hover:text-black">Beranda</Link>
          <span>/</span>
          ...
          <div className="flex flex-col">
            ...
            <div className="mt-auto space-y-3">
              ...
              <p className="text-center text-[11px] text-black/40">
                Link afiliasi · Mendukung konten kurasi ini
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="py-16 bg-black text-white border-t border-black/10">
        <div className="container-x">
          <h2 className="section-title mb-8">Kamu Mungkin Juga Suka</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {related.map(p => (
              <Link key={p.slug} href={`/ullys/p/${p.slug}`} className="group">
  ...
  return (
    <header className="sticky top-0 z-50 bg-white/90 backdrop-blur border-b border-black/5">
      <div className="container-x flex items-center justify-between py-4">
        ...
        <Link href={`/ullys/#semua`} className="btn-dark text-xs !py-2 !px-4">Lihat Koleksi</Link>
      </div>
    </header>
  );
}
